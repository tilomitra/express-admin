...
function initDatabase(args, done) {
	...
	async.series(
		[
			function(done) {
				var options =
					args.config.mysql || args.config.pg || args.config.sqlite;
				client.connect(
					options,
					function(err) {
			...
			function(done) {
				schema.getData(client, function(err, data) {
					...
					var fpath = path.join(args.dpath, "settings"),
...
function initSettings(args) {
	...
	// upload
	var upload =
		args.config.app.upload || path.join(__dirname, "public/upload");
	...
	if (!events.hasOwnProperty("preSave"))
		events.preSave = function(req, res, args, next) {
			next();
		};
	if (!events.hasOwnProperty("postSave"))
		events.postSave = function(req, res, args, next) {
			next();
		};
	if (!events.hasOwnProperty("preList"))
		events.preList = function(req, res, args, next) {
			next();
		};
	args.events = events;
	...
	for (var key in args.custom) {
		...
		if (assets.external) {
			args.libs.external.js = args.libs.external.js.concat(
				assets.external.js || [],
			);
			args.libs.external.css = args.libs.external.css.concat(
				assets.external.css || [],
			);
		}
	}
...
function initServer(args) {
			...
			serveStatic(
				(function() {
					var dpath = path.resolve(
						__dirname,
						"node_modules/express-admin-static",
					);
	...
	for (var key in args.custom) {
		...
		if (
			...
			!assets.local ||
			!assets.local.path ||
			!fs.existsSync(assets.local.path)
		)
			continue;
	...
	initCommandLine(args, function(err) {
		...
		initDatabase(args, function(err) {
			...
			app.listen(args.config.server.port, function() {
				console.log(
					"Express Admin listening on port".grey,
